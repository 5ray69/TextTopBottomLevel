# -*- coding: utf-8 -*
# module 078_09_text_top_bottom_ levels.py
import clr
...
from text_top_bottom_levels.user_form_top_bottom_levels import UserFormTopBottomLevels

# DESERIALIZATION, читаем файл
with open(IN[0].DirectoryName + r'\text_top_bottom_levels\user_define.json', 'r') as file:
    dict_from_json = json.load(file)

f = UserFormTopBottomLevels(dict_from_json)
f.ShowDialog()
...
# создаем файл json, существующий одноименный перезапишется
with open(IN[0].DirectoryName + r'\text_top_bottom_levels\user_define.json', 'w') as file:
    json.dump(f.dict_user_select, file, indent=4)
